...
public class OpenNestedProjectsHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		try {
			progressService.run(true, false, pm -> openAllProjects(project, pm));
	...
	private void openAllProjects(IProject project, IProgressMonitor pm) {
		final Consumer<IProject> projectConsumer = p -> {
			if (!p.isOpen()) {
				try {
					p.open(pm);
				} catch (CoreException e) {
					PluginActivator.getDefault().log(Status.ERROR, "Error opening projects.", e);
				}
			}
		};
